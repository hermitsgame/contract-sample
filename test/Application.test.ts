...
describe("Application unit test", () => {
  ...
  describe("updateFirmware", () => {
    ...
    it("onlyOperator", async () => {
      await expect(application.connect(attacker).updateFirmware(name, version, uri)).to.be.revertedWith(
        "caller is not the operator",
      )
    })

    it("success", async () => {
      await expect(application.connect(operator).updateFirmware(name, version, uri))
        .to.emit(application, "FirmwareUpdated")
        .withArgs(name, version, uri)
    })
  })
})
