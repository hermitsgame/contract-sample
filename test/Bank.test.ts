...
describe("Bank unit test", () => {
  ...
  describe("deposit", () => {
    it("invalid value", async () => {
      await expect(bank.connect(holder2).deposit()).to.be.revertedWith("invalid value")
    })

    it("success", async () => {
      const value = 1000000
      await expect(bank.connect(holder2).deposit({ value })).to.emit(bank, "Deposit").withArgs(holder2.address, value)
  ...
  describe("transfer", () => {
    it("onlyOperator", async () => {
      await expect(bank.connect(attacker).transfer(holder1.address, holder2.address, 10000)).to.be.revertedWith(
        "caller is not the operator",
      )
    })

    it("invalid amount", async () => {
      await expect(bank.connect(operator).transfer(holder1.address, holder2.address, 0)).to.be.revertedWith(
        "invalid amount",
      )
    })

    it("amount exceeds balance", async () => {
      await expect(bank.connect(operator).transfer(holder1.address, holder2.address, 100000000)).to.be.revertedWith(
        "amount exceeds balance",
      )
    })

    it("success", async () => {
      await expect(bank.connect(operator).transfer(holder1.address, holder2.address, 10000))
        .to.emit(bank, "Transfer")
        .withArgs(holder1.address, holder2.address, 10000)
    })
  })
